/*
 * pegatron_fm_6256_bn_f_sfp.c - A driver to read and write the EEPROM on optical transceivers
 * (SFP and QSFP)
 *
...
struct fm_6256_bn_f_sfp_platform_data {
...
struct fm_6256_bn_f_sfp_data {
	struct fm_6256_bn_f_sfp_platform_data chip[MAX_PORT_NUM];
...
extern int pegatron_fm_6256_bn_f_cpld_read(unsigned short cpld_addr, u8 reg);
extern int pegatron_fm_6256_bn_f_cpld_write(unsigned short cpld_addr, u8 reg, u8 value);
...
static uint8_t fm_6256_bn_f_sfp_translate_offset(struct fm_6256_bn_f_sfp_data *sfp,
		loff_t *offset, struct i2c_client **client, int num)
{
	...
	*offset = SFP_PAGE_SIZE + (*offset & 0x7f);
	...
	return page;  /* note also returning clifm_6256_bn_f_sfp_translate_offsetent and offset */
}

static ssize_t fm_6256_bn_f_sfp_eeprom_read(struct fm_6256_bn_f_sfp_data *sfp,
		    struct i2c_client *client,
...
static ssize_t fm_6256_bn_f_sfp_eeprom_write(struct fm_6256_bn_f_sfp_data *sfp,
				struct i2c_client *client,
				const char *buf,
...
static ssize_t fm_6256_bn_f_sfp_eeprom_update_client(struct fm_6256_bn_f_sfp_data *sfp,
				char *buf, loff_t off, size_t count, int num)
{
	...
	page = fm_6256_bn_f_sfp_translate_offset(sfp, &phy_offset, &client, num);

	dev_dbg(&client->dev,
		"%s off %lld  page:%d phy_offset:%lld, count:%ld\n",
		__func__, off, page, phy_offset, (long int) count);
	if (page > 0) {
		ret = fm_6256_bn_f_sfp_eeprom_write(sfp, client, &page,
	...
	while (count) {
		...
		status =  fm_6256_bn_f_sfp_eeprom_read(sfp, client,
 ...
 */
static ssize_t fm_6256_bn_f_sfp_page_legal(struct fm_6256_bn_f_sfp_data *sfp,
		loff_t off, size_t len, int num)
{
	...
	if (sfp->chip[num].dev_class == TWO_ADDR) {
		...
		status = fm_6256_bn_f_sfp_eeprom_read(sfp, client, &regval,
...
static ssize_t fm_6256_bn_f_sfp_read(struct fm_6256_bn_f_sfp_data *sfp,
		char *buf, loff_t off, size_t len, int num)
{
	 ...
	 */
	status = fm_6256_bn_f_sfp_page_legal(sfp, off, len, num);
	...
	for (chunk = off >> 7; chunk <= (off + len - 1) >> 7; chunk++) {
		 ...
		 */
		status = fm_6256_bn_f_sfp_eeprom_update_client(sfp, buf,
...
static ssize_t 
fm_6256_bn_f_sfp_bin_read(struct file *filp, struct kobject *kobj,
		struct bin_attribute *attr,
		char *buf, loff_t off, size_t count)
{
	int i;
	u8 cpldData = 0;
	struct fm_6256_bn_f_sfp_data *sfp;
	...
	for(i=0; i<CPLDB_SFP_NUM; i++)
	{
		...
		{
			pegatron_fm_6256_bn_f_cpld_write(CPLDB_ADDRESS, SFP_1_12_SCL_BASE, i+1);
			goto check_done;
		}
	}
	/*SFP 13-36*/
	for(i=0; i<CPLDA_SFP_NUM; i++)
	{
		...
		{
			pegatron_fm_6256_bn_f_cpld_write(CPLDA_ADDRESS, SFP_13_36_SCL_BASE, i+1);
			goto check_done;
		}
	}
	...
	for(i=0; i<CPLDC_SFP_NUM; i++)
	{
		if(!strcmp(attr->attr.name, SFP_CPLD_GROUPC_MAPPING[i]))
		{
			...
			{
				cpldData = 0xff;
				cpldData = pegatron_fm_6256_bn_f_cpld_read(CPLDC_ADDRESS, QSFP_I2C_ENABLE_BASE);
				CLEAR_BIT(cpldData, i-12);
				pegatron_fm_6256_bn_f_cpld_write(CPLDC_ADDRESS, QSFP_I2C_ENABLE_BASE, cpldData);
			}
			pegatron_fm_6256_bn_f_cpld_write(CPLDC_ADDRESS, SFP_37_54_SCL_BASE, i+1);
			goto check_done;
		}
	}

check_done:
	return fm_6256_bn_f_sfp_read(sfp, buf, off, count, i);
}
...
#define SFP_EEPROM_ATTR(_num)    \
        static struct bin_attribute sfp##_num##_eeprom_attr = {   \
                ...
                .read = fm_6256_bn_f_sfp_bin_read,   \
                }
...
static struct bin_attribute *fm_6256_bn_f_cpldA_sfp_epprom_attributes[] = {
...
static struct bin_attribute *fm_6256_bn_f_cpldB_sfp_epprom_attributes[] = {
...
static struct bin_attribute *fm_6256_bn_f_cpldC_sfp_epprom_attributes[] = {
...
static const struct attribute_group fm_6256_bn_f_sfpA_group = { .bin_attrs = fm_6256_bn_f_cpldA_sfp_epprom_attributes};
static const struct attribute_group fm_6256_bn_f_sfpB_group = { .bin_attrs = fm_6256_bn_f_cpldB_sfp_epprom_attributes};
static const struct attribute_group fm_6256_bn_f_sfpC_group = { .bin_attrs = fm_6256_bn_f_cpldC_sfp_epprom_attributes};

static int fm_6256_bn_f_sfp_device_probe(struct i2c_client *client, const struct i2c_device_id *dev_id)
{
	int i, err;
	struct fm_6256_bn_f_sfp_platform_data chip[MAX_PORT_NUM];
	struct fm_6256_bn_f_sfp_data *sfp;
	...
	sfp = kzalloc(sizeof(struct fm_6256_bn_f_sfp_data) +
	...
	switch(dev_id->driver_data)
	{
		case cpld_group_a:
			...
			err = sysfs_create_group(&client->dev.kobj, &fm_6256_bn_f_sfpA_group);
			if (err)
				goto err_clients;
			break;
		case cpld_group_b:
			...
			err = sysfs_create_group(&client->dev.kobj, &fm_6256_bn_f_sfpB_group);
			if (err)
				goto err_clients;
			break;
		case cpld_group_c:
			...
			err = sysfs_create_group(&client->dev.kobj, &fm_6256_bn_f_sfpC_group);
			if (err)
				goto err_clients;
			break;
		default:
...
static int fm_6256_bn_f_sfp_device_remove(struct i2c_client *client)
{
	struct fm_6256_bn_f_sfp_data *sfp;
	int i;
    ...
    {
        case cpld_group_a:
            sysfs_remove_group(&client->dev.kobj, &fm_6256_bn_f_sfpA_group);
            break;
        case cpld_group_b:
            sysfs_remove_group(&client->dev.kobj, &fm_6256_bn_f_sfpB_group);
            break;
        case cpld_group_c:
            sysfs_remove_group(&client->dev.kobj, &fm_6256_bn_f_sfpC_group);
            break;
        default:
...
static const struct i2c_device_id fm_6256_bn_f_sfp_id[] = {
    { "fm_6256_bn_f_sfpA", cpld_group_a },
    { "fm_6256_bn_f_sfpB", cpld_group_b },
    { "fm_6256_bn_f_sfpC", cpld_group_c },
    {}
};
MODULE_DEVICE_TABLE(i2c, fm_6256_bn_f_sfp_id);

static struct i2c_driver fm_6256_bn_f_sfp_driver = {
    .driver = {
        .name     = "pegatron_fm_6256_bn_f_sfp",
    },
    .probe        = fm_6256_bn_f_sfp_device_probe,
    .remove       = fm_6256_bn_f_sfp_device_remove,
    .id_table     = fm_6256_bn_f_sfp_id,
};

static int __init fm_6256_bn_f_sfp_init(void)
{
	int i;
	...
	return i2c_add_driver(&fm_6256_bn_f_sfp_driver);
}

static void __exit fm_6256_bn_f_sfp_exit(void)
{
	i2c_del_driver(&fm_6256_bn_f_sfp_driver);
}

MODULE_AUTHOR("Peter5 Lin <Peter5_Lin@pegatroncorp.com.tw>");
MODULE_DESCRIPTION("fm_6256_bn_f_sfp driver");
MODULE_LICENSE("GPL");

module_init(fm_6256_bn_f_sfp_init);
module_exit(fm_6256_bn_f_sfp_exit);
